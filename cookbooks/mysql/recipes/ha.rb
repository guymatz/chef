...
Chef::Log.info "mysql -u root -p#{node[:mysql][:server_root_password]} -e \"CHANGE MASTER TO master_host='#{cluster_slaves.first[:ipaddress]}', master_port=3306, master_user='repl', master_password='#{cluster_slaves.first[:mysql][:server_repl_password]}', master_log_file='#{master_log_file}', master_log_pos=#{master_log_pos};\""

Chef::Log.info "mysql -u root -p#{node[:mysql][:server_root_password]} -e \"START SLAVE;\""
