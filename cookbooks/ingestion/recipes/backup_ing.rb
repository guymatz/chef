...
cron_d "Move_backup_to_NFS" do
  command "/bin/cp $(/usr/bin/find /data/backups/ -name '*archwal.tar.gz' -type f -printf '\\%T@ \\%p\\n' | sort -n | tail -1 | cut -f2- -d\" \") /data/nfs/files.ihrdev.com/ing_bak; /bin/cp $(/usr/bin/find /data/backups/ -name '*data.tar.gz' -type f -printf '\\%T@ \\%p\\n' | sort -n | tail -1 | cut -f2- -d\" \") /data/nfs/files.ihrdev.com/ing_bak; /bin/cp $(/usr/bin/find /data/backups/ -name '*index.tar.gz' -type f -printf '\\%T@ \\%p\\n' | sort -n | tail -1 | cut -f2- -d\" \") /data/nfs/files.ihrdev.com/ing_bak;"
  minute  "25"
  hour    "4"
  month   "*"
  weekday "0,3,5"
