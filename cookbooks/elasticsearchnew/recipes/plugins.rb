...
unless tagged?("es-plugins-installed")
  ...
  if /stage/ =~ node.chef_environment
    ...
    command "#{ES_HOME}/bin/plugin --url #{node[:elasticsearchnew][:url]}/es-plugins/elasticsearch-river-rabbitmq.zip --install river-rabbitmq"
    cwd Chef::Config[:file_cache_path]
    notifies :restart, "service[elasticsearch]"
    user node[:elasticsearchnew][:user]
    group node[:elasticsearchnew][:group]
  ...
  if /primary_node/ =~ node[:hostname]
    execute "configure-river-rabbitmq-plugin" do
    command <<-EOH /usr/bin/curl -XPUT 'node[:ipaddress]:9200/_river/my_river/_meta' -d '{ 
      "type" : "rabbitmq", 
      "rabbitmq" : { 
          "addresses" : [
              {
