if ((/prod/ =~ node.chef_environment && (not tagged?("event_job_deployed")))) || (/prod/ !~ node.chef_environment)
  ...
  remote_file "/data/jobs/event/event_job.jar" do
    source "http://files.ihrdev.com/jobs/event/#{node.chef_environment}/event_job.jar"
  end
