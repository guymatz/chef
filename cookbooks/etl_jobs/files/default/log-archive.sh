...
nice -n 19 tar czf /data/log/customradiothumbslog/processed/processed_customthumbslogs_10.90.40.6_`/bin/date +'\%H-\%d-\%m-\%Y'.tar.gz` `/usr/bin/find /data/log/customradiothumbslog/processed/ -name -mmin +120`;nice -n 19 /usr/bin/find /data/log/customradiothumbslog/processed/ -mmin +120 -exec rm -f {} \;

nice -n 19 tar czf /data/log/liveradiothumbslog/processed/processed_livethumbslogs_10.90.40.6_`/bin/date +'\%H-\%d-\%m-\%Y'.tar.gz` `/usr/bin/find /data/log/liveradiothumbslog/processed/ -name -mmin +120`;nice -n 19 /usr/bin/find /data/log/liveradiothumbslog/processed/ -mmin +120 -exec rm -f {} \;
