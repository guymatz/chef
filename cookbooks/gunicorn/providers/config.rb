...
action :create do
  ...
  template_variables = {}
  %w{listen backlog preload_app worker_processes worker_class worker_timeout worker_keepalive worker_max_requests server_hooks pid stderr_logfile stdout_logfile }.each do |a|
    template_variables[a.to_sym] = new_resource.send(a)
  end
  puts "supervisor template vars=" + template_variables.inspect
