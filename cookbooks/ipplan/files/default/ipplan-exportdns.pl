...
# used in &query_db
my $DSN = 'dbi:mysql:ipplan:host=use1b-ss-db001a:port=3306';
my $DBUSER = 'ipplan';
my $DBPASS = 'ipplanpw';
...
my @fwd_script = ('curl', "-u $IPPLAN_USER:$IPPLAN_USER", 'http://use1b-ss-admin001/user/modifydns.php?');
my $fwd_query_base = "action=export&cust=$customer";
#my @rev_script = ('php-cgi', '/data/www/ipplan/user/modifyzone.php');
my @rev_script = ('curl', "-u $IPPLAN_USER:$IPPLAN_USER", 'http://use1b-ss-admin001/user/modifyzone.php?');
